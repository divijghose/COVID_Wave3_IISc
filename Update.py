...
#################################### Read SSE till last day ######################################
with open('Data_Files/sse.pkl', 'rb') as f:
    SSE = pickle.load(f)

lastDay = pd.read_pickle('Data_Files/day.pkl')
###################################### Update ###################################

N = 972 #No. of realizations
...
for i in range(N):
    for j in range(lastDay+1,len(actual_cases['Actual'])-D):
...
###################################### Update Input Files ###################################
with open('Data_Files/sse.pkl', 'wb') as f:
    pickle.dump(SSE, f)
